...
{
    public class BusinessValidationsMastercard : IBusinessValidations
    {
        public GenericReponse<bool> IsValidAmount(DeferPaymentType deferralPayment, decimal amount)
        {
            switch (deferralPayment)
            {
                ...
                case DeferPaymentType.SixMonths:
                    return new GenericReponse<bool> { Data = false, Error = new Model.Error { ValidationErrors = new List<string> { "El Pago minimo es de $600.00 MXN." } } };
                case DeferPaymentType.NineMonths:
                    return new GenericReponse<bool> { Data = false, Error = new Model.Error { ValidationErrors = new List<string> { "El Pago minimo es de $900.00 MXN." } } };
